def recurse(x1, x2, y1, y2):
	if x2 - x1 == 1:
		...
		return grid[y1][x1] == "1", grid[y1][x1] == "0", 0
	size = x2 - x1 >> 1
	xmid = x1 + size
	ymid = y1 + size
	...
	cost = 262144
	for perm in __import__("itertools").permutations(xrange(4)):
		newcost = squares[perm[0]][0] + squares[perm[1]][1] + squares[perm[2]][2] + squares[perm[3]][2]
		if newcost < cost:
			cost = newcost
			best = perm
